...
const ResponsibleTecnic: React.FC<FormClinicProfileI> = (props: FormClinicProfileI) => {
  ...
  /** Context */
  const { error, setIsHashModificationField } = React.useContext(ClinicEditContext)

  /** @description Atualiza os dados digitados nos states */
  React.useEffect(() => {
    props.setResponsibleTecnic({ responsibleTecnic: responsible,
  ...
  return (
    <Container>
      <h1>Responsável Técnico</h1>
       <section>
        <InputText label='Responsável Técnico*'
                   disabled={isDisabled}
