...
const EditUsersClinic: React.FC = () => {
  ...
  const [erros, setError] = React.useState<ValidationErrorFieldsI>(
    {} as ValidationErrorFieldsI,
  )
  ...
  const getUserClinicById = async () => {
    const result = await apiAdmin.get(
      `/clinica/${location.state.idClinica}/usuario/${location.state.idUsuario}`,
    )
          ...
          <section>
            {!isEditing ? (
              <>
                ...
                <ButtonEdit setIsEditing={setIsEditing} />
              </>
            ) : (
              <>
                <ButtonCancel
